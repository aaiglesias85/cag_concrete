...
INSERT INTO `gender` (`gender_id`, `code`, `description`, `classification`) VALUES
...
(7, 'Oth', 'Other', 'Not a minority');
  ...
  INSERT INTO `function` (`function_id`, `url`, `description`) VALUES ('34', 'gender', 'Genders');
   ...
   ADD `social_security_number` VARCHAR(50) NULL AFTER `cert_rate_type`,
    ADD `apprentice_percentage` DECIMAL(18,2) NULL AFTER `social_security_number`,
     ADD `work_code` VARCHAR(50) NULL AFTER `apprentice_percentage`,
      ADD `ethnicity_race` VARCHAR(255) NULL AFTER `work_code`;


ALTER TABLE `employee` ADD `gender_id` INT(11) NULL AFTER `ethnicity_race`, ADD INDEX (`gender_id`);

ALTER TABLE `employee` ADD CONSTRAINT `Refemployee1` FOREIGN KEY (`gender_id`)
    REFERENCES `gender`(`gender_id`) ON DELETE RESTRICT ON UPDATE RESTRICT;


ALTER TABLE `employee` ADD `date_hired` DATE NULL AFTER `gender_id`,
