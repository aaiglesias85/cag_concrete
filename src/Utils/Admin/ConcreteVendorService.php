...
use App\Repository\ConcreteVendorContactRepository;
use App\Repository\ConcreteVendorRepository;
...
class ConcreteVendorService extends Base
{
    ...
    public function EliminarContact($contact_id)
    {
        ...
        if (null != $entity) {
            ...
            $schedules = $scheduleConcreteVendorContactRepo->ListarSchedulesDeContact($contact_id);
    ...
    public function ListarOrdenados()
    {
        ...
        $lista = $concreteVendorRepo->ListarOrdenados();
    ...
    public function ListarProjects($vendor_id)
    {
        ...
        $vendor_projects = $dataTrackingConcVendorRepo->ListarProjectsDeConcVendor($vendor_id);
    ...
    public function EliminarVendor($vendor_id)
    {
        ...
        if (null != $entity) {
            ...
            $projects = $projectRepo->ListarProjectsDeConcVendor($vendor_id);
            if (count($projects) > 0) {
                $resultado['success'] = false;
    ...
    public function EliminarInformacionRelacionada($vendor_id)
    {
        ...
        // contacts
        /** @var ConcreteVendorContactRepository $concreteVendorContactRepo */
        $concreteVendorContactRepo = $this->getDoctrine()->getRepository(ConcreteVendorContact::class);
        $contacts = $concreteVendorContactRepo->ListarContacts($vendor_id);
        foreach ($contacts as $contact) {
            // schedules
            /** @var ScheduleConcreteVendorContactRepository $scheduleConcreteVendorContactRepo */
            $scheduleConcreteVendorContactRepo = $this->getDoctrine()->getRepository(ScheduleConcreteVendorContact::class);
            $schedules_contact = $scheduleConcreteVendorContactRepo->ListarSchedulesDeContact($contact->getContactId());
        ...
        /** @var DataTrackingConcVendorRepository $dataTrackingConcVendorRepo */
        $dataTrackingConcVendorRepo = $this->getDoctrine()->getRepository(DataTrackingConcVendor::class);
        $data_trackings = $dataTrackingConcVendorRepo->ListarDataTrackingsDeConcVendor($vendor_id);
        ...
        $schedules = $scheduleRepo->ListarSchedulesDeConcreteVendor($vendor_id);
        foreach ($schedules as $schedule) {
    ...
    public function ActualizarVendor($vendor_id, $name, $phone, $address, $contactName, $contactEmail, $contacts)
    {
        ...
        if (null != $entity) {
            ...
            $vendor = $this->getDoctrine()->getRepository(ConcreteVendor::class)
               ->findOneBy(['name' => $name]);
            if (null != $vendor && $entity->getVendorId() != $vendor->getVendorId()) {
                $resultado['success'] = false;
