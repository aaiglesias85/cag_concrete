...
class EstimateService extends Base
{
    ...
    public function SalvarQuoteCompanies($quote_id, $estimate_company_ids)
    {
        ...
        $ids = is_array($estimate_company_ids) ? $estimate_company_ids : (false !== strpos((string) $estimate_company_ids, ',') ? explode(',', $estimate_company_ids) : [$estimate_company_ids]);
    ...
    public function EnviarQuotes($quote_ids)
    {
        ...
        foreach ($ids as $quote_id) {
            ...
            if (null === $pdfPath || !is_file($pdfPath)) {
    ...
    private function buildSpreadsheetForQuote($quote_id)
    {
        ...
        $templateName = ($proposalType && 2 === (int) $proposalType->getTypeId()) ? 'bid_letting.xlsx' : 'bid_bids.xlsx';
        $templatePath = $this->getParameter('kernel.project_dir').DIRECTORY_SEPARATOR.'public'.DIRECTORY_SEPARATOR.'assets'.DIRECTORY_SEPARATOR.'metronic8'.DIRECTORY_SEPARATOR.'excel'.DIRECTORY_SEPARATOR.$templateName;
        ...
        $firstDataRow = ('bid_letting.xlsx' === $templateName) ? 11 : 12;
        ...
        foreach ($items as $row) {
            ...
            foreach ($quoteItemNotes as $eqin) {
                $n = $eqin->getNoteItem();
                if (null !== $n && null !== $n->getDescription() && '' !== $n->getDescription()) {
        ...
        $notesRowBase = ('bid_letting.xlsx' === $templateName) ? 41 : 42;
    ...
    public function AgregarItem($estimate_item_id, $estimate_id, $quote_id, $item_id, $item_name, $unit_id, $quantity, $price, $yield_calculation, $equation_id, $note_ids = [], $code = null, $contract_name = null, ?string $new_quote_name = null)
    {
        ...
        if ('' !== $item_id) {
            $quote_id_check = ('' !== $quote_id && null !== $quote_id && is_numeric($quote_id)) ? $quote_id : null;
            if (null !== $quote_id_check) {
                /** @var EstimateQuoteItemRepository $estimateQuoteItemRepo */
                $estimateQuoteItemRepo = $this->getDoctrine()->getRepository(EstimateQuoteItem::class);
        ...
        if (null != $estimate_entity) {
            $newQuoteNameTrim = $this->normalizeNullableTrimmedString($new_quote_name);
            $newQuoteNameTrim = (null !== $newQuoteNameTrim && '' !== $newQuoteNameTrim) ? $newQuoteNameTrim : null;
            ...
            if ('' !== $quote_id && is_numeric($quote_id)) {
                ...
                if (null !== $quote_entity && $quote_entity->getEstimate()->getEstimateId() == (int) $estimate_id) {
                    $default_quote = $quote_entity;
                }
            }

            if (null === $default_quote && null !== $newQuoteNameTrim) {
    ...
    public function EliminarItem($estimate_item_id)
    {
        ...
        if (null != $entity) {
            ...
            $quoteIdAfterDelete = null !== $quoteAfterDelete ? $quoteAfterDelete->getId() : null;
    ...
    public function CargarDatosEstimate($estimate_id)
    {
        ...
        if (null != $entity) {
            ...
            $arreglo_resultado['stage_id'] = null != $entity->getStage() ? $entity->getStage()->getStageId() : '';
            $arreglo_resultado['proposal_type_id'] = null != $entity->getProposalType() ? $entity->getProposalType()->getTypeId() : '';
            $arreglo_resultado['status_id'] = null != $entity->getStatus() ? $entity->getStatus()->getStatusId() : '';
            ...
            $arreglo_resultado['district_id'] = null != $entity->getDistrict() ? $entity->getDistrict()->getDistrictId() : '';
            $arreglo_resultado['plan_downloading_id'] = null != $entity->getPlanDownloading() ? $entity->getPlanDownloading()->getPlanDownloadingId() : '';
    ...
    public function AgregarTemplateNote($estimate_id, $estimate_note_item_id)
    {
        ...
        if (null === $estimate || null === $noteItem) {
            return ['success' => false, 'error' => 'Estimate or note not found'];
        }
        if ('template' !== $noteItem->getType()) {
    ...
    public function DevolverItemDeEstimate($value, $key = -1)
    {
        ...
        return [
            ...
            'quote_id' => null !== $value->getQuote() ? $value->getQuote()->getId() : null,
    ...
    public function ActualizarEstimate(
        ...
        if (null != $entity) {
            ...
            $estimate = $this->getDoctrine()->getRepository(Estimate::class)
               ->findOneBy(['name' => $name]);
            if (null != $estimate && $estimate_id != $estimate->getEstimateId()) {
                $resultado['success'] = false;
    ...
    public function SalvarCompanys($entity, $companys)
    {
        ...
        if (!empty($companys)) {
            foreach ($companys as $value) {
                ...
                if (null == $estimate_company_entity) {
                ...
                $tag = $value->tag ?? null;
                $estimate_company_entity->setTag('' !== $tag && null !== $tag ? (string) $tag : null);

                $address = $value->address ?? null;
                $estimate_company_entity->setAddress('' !== $address && null !== $address ? (string) $address : null);
    ...
    {
        if (null === $companys || !is_iterable($companys)) {
            return null;
        }
        $max = null;
        foreach ($companys as $value) {
    ...
    public function SalvarCounties(Estimate $entity, $counties_id, bool $is_new = true): void
    {
        ...
        if (null === $counties_id || '' === $counties_id) {
            return;
        }
    ...
    public function ListarEstimatesParaCalendario(
        ...
        $sSearch = null !== $search && '' !== $search ? (string) $search : '';

        if ('' === $project_type_id || null === $project_type_id) {
            /** @var EstimateRepository $estimateRepo */
            $estimateRepo = $this->getDoctrine()->getRepository(Estimate::class);
        ...
        foreach ($lista as $entity) {
            ...
            $stageColor = null !== $stage ? $stage->getColor() : null;
            $stageName = null !== $stage ? (string) $stage->getDescription() : '';
