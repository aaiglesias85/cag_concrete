...
class DistrictService extends Base
{
    ...
    private function SePuedeEliminarDistrict($district_id)
    {
        ...
        // countys
        $countys = $this->getDoctrine()->getRepository(County::class)
            ->ListarCountysDeDistrict($district_id);
        if (count($countys) > 0) {
    ...
    {
        $resultado = $this->getDoctrine()->getRepository(District::class)
            ->ListarDistrictsConTotal($start, $limit, $sSearch, $iSortCol_0, $sSortDir_0);
