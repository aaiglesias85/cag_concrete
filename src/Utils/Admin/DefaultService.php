...
class DefaultService extends Base
{
   ...
   public function ListarMaterialsConMontos($project_id = '', $fecha_inicial = '', $fecha_fin = '', $status = '')
   {
      ...
      foreach ($materials as $material) {
         ...
         $quantity = $dataTrackingMaterialRepo->TotalQuantity('', $project_id, $material_id, $fecha_inicial, $fecha_fin, $status);
         $amount = $dataTrackingMaterialRepo->TotalMaterials('', $material_id, $project_id, $fecha_inicial, $fecha_fin, $status);
   ...
   public function ListarItemsConMontos($project_id = '', $fecha_inicial = '', $fecha_fin = '', $status = '')
   {
      ...
      foreach ($project_items as $project_item) {
         ...
         $quantity = $dataTrackingItemRepo->TotalQuantity('', $project_item_id, $fecha_inicial, $fecha_fin, $status);

         /** @var DataTrackingItemRepository $dataTrackingItemRepo */
         $dataTrackingItemRepo = $this->getDoctrine()->getRepository(DataTrackingItem::class);
         $amount = $dataTrackingItemRepo->TotalDaily('', $project_item_id, '', $fecha_inicial, $fecha_fin, $status);
   ...
   public function DevolverDataChart3($project_id = '', $fecha_inicial = '', $fecha_fin = '', $status = '')
   {
      ...
      foreach ($invoices as $invoice) {
         ...
         /** @var InvoiceItemRepository $invoiceItemRepo */
         $invoiceItemRepo = $this->getDoctrine()->getRepository(InvoiceItem::class);
         $amount = $invoiceItemRepo->TotalInvoice($invoice->getInvoiceId());
   ...
   private function CalcularProfitTotal($project_id = '', $fecha_inicial = '', $fecha_fin = '', $status = '')
   {
      ...
      foreach ($projects as $project) {
         ...
         /** @var DataTrackingItemRepository $dataTrackingItemRepo */
         $dataTrackingItemRepo = $this->getDoctrine()->getRepository(DataTrackingItem::class);
         $total_daily_today = $dataTrackingItemRepo->TotalDaily('', '', $project_id, $fecha_inicial, $fecha_fin, $status);
         ...
         $total_subcontract = $dataTrackingSubcontractRepo->TotalPrice('', '', $project_id, $fecha_inicial, $fecha_fin, $status);
