...
class AdvertisementService extends Base
{
    ...
    public function ListarAdvertisementsUltimosDias()
    {
        ...
        $lista = $this->getDoctrine()->getRepository(Advertisement::class)
            ->ListarOrdenados($fecha_actual, $fecha_actual);
    ...
    public function ListarAdvertisements($start, $limit, $sSearch, $iSortCol_0, $sSortDir_0, $fecha_inicial, $fecha_fin)
    {
        ...
        $lista = $this->getDoctrine()->getRepository(Advertisement::class)
            ->ListarAdvertisements($start, $limit, $sSearch, $iSortCol_0, $sSortDir_0, $fecha_inicial, $fecha_fin);

        foreach ($lista as $value) {
            $advertisement_id = $value->getAdvertisementId();

            $acciones = $this->ListarAcciones($advertisement_id);
    ...
    /**
     * TotalAdvertisements: Total de advertisements
     * @param string $sSearch Para buscar
     * @author Marcel
     */
    public function TotalAdvertisements($sSearch, $fecha_inicial, $fecha_fin)
    {
        $total = $this->getDoctrine()->getRepository(Advertisement::class)
            ->TotalAdvertisements($sSearch, $fecha_inicial, $fecha_fin);
    ...
    public function ListarAcciones($id)
    {
        ...
        $permiso = $this->BuscarPermiso($usuario->getUsuarioId(), 17);
