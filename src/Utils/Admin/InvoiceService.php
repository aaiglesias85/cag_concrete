...
class InvoiceService extends Base
{
    ...
    public function ValidarInvoice($invoice_id, $project_id, $start_date, $end_date, $number)
    {
        ...
        if ('' !== $number) {
            ...
            if (null != $invoice && $invoice->getInvoiceId() != $invoice_id) {
    ...
    public function ChangeNumber($invoice_id, $number)
    {
        ...
        if (null !== $entity) {
            ...
            if (null != $invoice && $invoice->getInvoiceId() != $entity->getInvoiceId()) {
    ...
    public function ExportarExcel($invoice_id)
    {
        ...
        $templatePath = $this->getParameter('kernel.project_dir').DIRECTORY_SEPARATOR.'public'.DIRECTORY_SEPARATOR.'assets'.DIRECTORY_SEPARATOR.'metronic8'.DIRECTORY_SEPARATOR.'excel'.DIRECTORY_SEPARATOR.'invoice.xlsx';
        ...
        for ($i = $start_row_data; $i < 200; ++$i) {
            ...
            if (false !== strpos($valG, 'CONTRACT AMOUNT') || false !== strpos($valE, 'TOTAL')) {
                $fila_footer_inicio = $i;
                break;
            }
        }
        if (0 == $fila_footer_inicio) {
            $fila_footer_inicio = 41;
        }
        ...
        $aplicarFormatoFila = function ($sheet, $f) use ($styleLeft, $styleRight, $currencyFormat, $qtyFormat) {
            ...
            foreach ($colsMoney as $col) {
                $sheet->getStyle($col.$f)->getNumberFormat()->setFormatCode($currencyFormat);
            }
            $colsQty = ['G', 'I', 'K', 'M', 'O', 'Q', 'R'];
            foreach ($colsQty as $col) {
                $sheet->getStyle($col.$f)->getNumberFormat()->setFormatCode($qtyFormat);
            }
        };
        ...
        if ($last_data_row < $start_row_data) {
            $last_data_row = $start_row_data;
        }
    ...
    public function ExportarPdf($invoice_id)
    {
        ...
        $logoFullPath = str_replace('\\', '/', $publicDir.'/assets/metronic8/img/logo.jpg');
    ...
    public function ObtenerDatosExportacionInvoice($invoice_id)
    {
        ...
        $hay_bond = (null !== $bondInvoiceItem || null !== $bondInvoiceItemFromCO || !empty($bondProjectItems));
        ...
        if ($hay_bond) {
            $bon_qty = null !== $invoice_entity->getBonQuantity() ? (float) $invoice_entity->getBonQuantity() : 0.0;
            $bon_amt = null !== $invoice_entity->getBonAmount() ? (float) $invoice_entity->getBonAmount() : 0.0;
            $prev_bon_qty = 0.0;
            $prev_bon_amt = 0.0;
        ...
        $percentage_used = ($invoice_current_base > 0 && 0.0 !== $current_retainage_amount)
    ...
    public function PrepararDatosParaPdfTemplate($invoice_id)
    {
        ...
        $projectDir = $this->getParameter('kernel.project_dir');
        $logoPath = str_replace('\\', '/', $projectDir.DIRECTORY_SEPARATOR.'public'.DIRECTORY_SEPARATOR.'assets'.DIRECTORY_SEPARATOR.'metronic8'.DIRECTORY_SEPARATOR.'img'.DIRECTORY_SEPARATOR.'logo.jpg');
    ...
    private function getItemDisplayNameForExport(ProjectItem $projectItem): string
    {
        ...
        $name = null !== $item && null !== $item->getName() ? trim((string) $item->getName()) : '';

        $codeRaw = $projectItem->getCode();
        $code = null !== $codeRaw && '' !== trim((string) $codeRaw) ? trim((string) $codeRaw) : '';

        $cnRaw = $projectItem->getContractName();
        $second = (null !== $cnRaw && '' !== trim((string) $cnRaw)) ? trim((string) $cnRaw) : $name;
    ...
    public function CalcularRetainageEfectivoParaInvoice($invoice_id): array
    {
        ...
        foreach ($allInvoices as $inv) {
            if (null === $inv->getInvoiceRetainageCalculated()) {
                $this->CalcularYGuardarRetainageInvoice($inv);
                $em->flush();
                $em->refresh($inv);
            }
    ...
    public function CargarDatosInvoice($invoice_id)
    {
        ...
        if (null != $entity) {
            ...
            $arreglo_resultado['bon_quantity'] = null !== $entity->getBonQuantity() ? (float) $entity->getBonQuantity() : null;
            $arreglo_resultado['bon_amount'] = null !== $entity->getBonAmount() ? (float) $entity->getBonAmount() : null;
    ...
    public function ListarItemsDeInvoice($invoice_id)
    {
        ...
        $this->logOverrideInvoice('listar_items_start', [
            ...
            'project_id' => $project_id,
            'invoice_period_start' => null !== $invSd ? $invSd->format('Y-m-d') : null,
            'invoice_period_end' => null !== $invEd ? $invEd->format('Y-m-d') : null,
        ...
        foreach ($lista as $key => $value) {
            ...
            if (null !== $invPeriodStart && null !== $invPeriodEnd) {
            ...
            $this->logOverrideInvoice('item_override_row', [
                ...
                'period_override_row_id' => null !== $periodOverrideRow ? $periodOverrideRow->getId() : null,
                'latest_override_id' => null !== $latestOverride ? $latestOverride->getId() : null,
                'override_period_date_ymd' => null !== $overrideStartDate ? $overrideStartDate->format('Y-m-d') : null,
                'override_partition_date_ymd' => null !== $overridePartitionDate ? $overridePartitionDate->format('Y-m-d') : null,
                'override_snapshot_unpaid_column' => null !== $latestOverride ? $latestOverride->getUnpaidQty() : null,
                'anchor_unpaid_effective' => $anchorUnpaidEffective,
                'override_snapshot_paid' => null !== $periodOverrideRow ? $periodOverrideRow->getPaidQty() : (null !== $latestOverride ? $latestOverride->getPaidQty() : null),
            ]);
            ...
            foreach ($allInvoices as $inv) {
                ...
                if (!$isAfterOverride) {
                    ...
                    if (
                        ...
                        if (null !== $lastUnpaidOverrideValue) {
                ...
                } elseif (null !== $lastUnpaidOverrideValue) {
                ...
                if ($loopInvId === $currentInvoiceId) {
                    ...
                    if (!$isAfterOverride) {
                        ...
                        if (
                            ...
                            $unpaidBeforeQbfForRow = null !== $carryAtLoopStart
                               ? (float) $carryAtLoopStart
                               : (float) $anchorUnpaidEffective;
                        } elseif (null !== $carryAtLoopStart) {
                    ...
                    $this->logQbf('listar_timeline_current_invoice', [
                        ...
                        'same_month_as_override_header' => null !== $invStart && null !== $overrideStartDate
                           && $this->isSameCalendarMonth($invStart, $overrideStartDate),
                    ]);
                    $this->logOverrideInvoice('timeline_at_current_invoice', [
                        ...
                        'branch' => !$isAfterOverride
                           ...
                           : (null !== $latestOverride
                              ...
                              : (null !== $lastUnpaidOverrideValue ? 'propagate_last' : 'calculate_invoice_unpaid_qty')),
                        'i_qty' => $iQty,
                        'i_paid_timeline' => $iPaid,
                        'current_qbf' => $currentQbf,
                    ]);
                    $unpaidQtySpecific = $unpaidOpeningForDisplay;
                    $unpaidPrevSpecific = null !== $carryAtLoopStart
                       ? (float) $carryAtLoopStart
                       : $lastLoopUnpaid;
                    $foundSpecific = true;
                }
            ...
            if (!$foundSpecific) {
                ...
                if ($isAfterOverride && null !== $latestOverride && null !== $anchorUnpaidEffective) {
                    ...
                    if (
                        null !== $currentInvStart && null !== $overrideStartDate
                        && $this->isSameCalendarMonth($currentInvStart, $overrideStartDate)
                    ) {
                        $unpaidBeforeQbf = null !== $carryIntoCurrentForFallback
                           ? (float) $carryIntoCurrentForFallback
                           : $overrideBase;
                    } elseif (null !== $carryIntoCurrentForFallback) {
            ...
            if (null !== $invPeriodStart && null !== $invPeriodEnd
               && null !== $latestOverride && null !== $anchorUnpaidEffective
            ) {
            ...
            if (($pd['override_id'] ?? null) !== null
               && null !== $periodOverrideRow
               && null === $periodOverrideRow->getUnpaidQty()
            ) {
                $qbfRow = (float) ($value->getQuantityBroughtForward() ?? 0);
    ...
    public function ListarInvoices($start, $limit, $sSearch, $iSortCol_0, $sSortDir_0, $company_id, $project_id, $fecha_inicial, $fecha_fin)
    {
        ...
        foreach ($resultado['data'] as $value) {
            ...
            $data[] = [
                ...
                'hasOverride' => (null !== $value->getStartDate() && null !== $value->getEndDate())
    ...
    private function ActualizarUnpaidQtyPorQuantityBroughtForward($currentInvoice, $items)
    {
        ...
        foreach ($items as $itemData) {
            ...
            if (null === $project_item_id || '' === $project_item_id) {
                continue;
            }

            $projectItem = $this->getDoctrine()->getRepository(ProjectItem::class)->find($project_item_id);
            if (null === $projectItem) {
                continue;
            }
            ...
            $this->logOverrideInvoice('actualizar_unpaid_qbf_project_item', [
                ...
                'anchor_unpaid_effective' => $anchorUnpaidEffective,
                'override_partition_ymd' => null !== $overridePartitionDate ? $overridePartitionDate->format('Y-m-d') : null,
                'override_start_ymd' => null !== $overrideStartDate ? $overrideStartDate->format('Y-m-d') : null,
                'latest_override_id' => null !== $latestOverride ? $latestOverride->getId() : null,
            ]);
            ...
            for ($i = 0; $i < count($allInvoices); ++$i) {
                ...
                $nuevoUnpaid = null;
                if (null !== $latestOverride && null !== $anchorUnpaidEffective) {
                    $overrideBase = (float) $anchorUnpaidEffective;
                    if (
                        null !== $invStart && null !== $overrideStartDate
                        && $this->isSameCalendarMonth($invStart, $overrideStartDate)
    ...
    public function RecalcularUnpaidQtyProyecto(int $project_id): void
    {
        ...
        foreach ($projectItemIds as $project_item_id) {
            ...
            for ($i = 0; $i < count($allInvoices); ++$i) {
                ...
                $recalcOverrideStart = null !== $recalcAnchor ? $recalcAnchor->getOverridePeriodDate() : null;

                if (null !== $recalcAnchor && null !== $recalcAnchorUnpaidEffective) {
                    $overrideBase = (float) $recalcAnchorUnpaidEffective;
                    if (
                        null !== $invStart && null !== $recalcOverrideStart
                        && $this->isSameCalendarMonth($invStart, $recalcOverrideStart)
                    ) {
                        $nuevoUnpaid = max(0.0, $overrideBase - $currentQbf);
                        $lastUnpaidOverrideCarry = max(0.0, $overrideBase + $iQty - $currentQbf);
                    } else {
                        if (null !== $lastUnpaidOverrideCarry) {
    ...
    public function ObtenerSiguientePeriodoInvoice($project_id): array
    {
        ...
        if (null !== $lastInvoice) {
            ...
            if (null !== $lastStart && null !== $lastEnd) {
