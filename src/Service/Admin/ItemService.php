...
class ItemService extends Base
{
    ...
    public function CargarDatosItem($item_id)
    {
        ...
        if (null != $entity) {
            ...
            $arreglo_resultado['unit_id'] = null != $entity->getUnit() ? $entity->getUnit()->getUnitId() : '';
            $arreglo_resultado['yield_calculation'] = $entity->getYieldCalculation();
            $arreglo_resultado['equation_id'] = null != $entity->getEquation() ? $entity->getEquation()->getEquationId() : '';
    ...
    public function ListarProjectsDeItem($item_id)
    {
        ...
        foreach ($lista as $key => $value) {
            $project = [
                ...
                'county' => $this->getCountiesDescriptionForProject($value->getProject()),
                'posicion' => $key,
            ];
            $projects[] = $project;
        }
    ...
    public function ActualizarItem($item_id, $unit_id, $name, $description, $status, $bond, $yield_calculation, $equation_id)
    {
        ...
        if (null != $entity) {
            ...
            $item = $this->getDoctrine()->getRepository(Item::class)
               ->findOneBy(['name' => $name]);
            if (null != $item && $entity->getItemId() != $item->getItemId()) {
                $resultado['success'] = false;
