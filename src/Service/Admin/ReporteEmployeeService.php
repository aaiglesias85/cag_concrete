...
class ReporteEmployeeService extends Base
{
    ...
    public function ExportarExcel($search, $employee_id, $project_id, $fecha_inicial, $fecha_fin)
    {
        ...
        $spreadsheet = $reader->load($this->getParameter('kernel.project_dir').DIRECTORY_SEPARATOR.'public'.DIRECTORY_SEPARATOR.'assets'.DIRECTORY_SEPARATOR.'metronic8'.DIRECTORY_SEPARATOR.'excel'.DIRECTORY_SEPARATOR.'report-employee.xlsx');
        ...
        $empleadosNormales = array_filter($employees, fn ($e) => 'Subcontractor' !== $e['role']);
        $subcontractors = array_filter($employees, fn ($e) => 'Subcontractor' === $e['role']);
        ...
        foreach ($semanas as $semana) {
            ...
            foreach ($empleadosOrdenados as $employee) {
                ...
                $isSubcontractor = 'Subcontractor' === $employee['role'];
                ...
                foreach ($semana->dias as $index => $dia) {
                    ...
                    $proyectos = array_map(
                        ...
                        $dataTrackingLaborRepo->ListarReporteEmployeesParaExcel($search, $employee['employee_id'], $project_id, $dia, $dia)
                    );

                    // buscar el color del dia
                    $labores = $dataTrackingLaborRepo->ListarEmployeesDeProject($project_id, $dia, $dia, $employee['employee_id']);
                    ...
                    $horas = $dataTrackingLaborRepo->TotalHours('', $employee['employee_id'], $project_id, $dia, $dia);
            ...
            $ultimaColumna = $col;
            $rangoFilaTotal = 'A'.$fila.':'.Coordinate::stringFromColumnIndex($ultimaColumna).$fila;
    ...
    public function ListarEmployeesParaReporteExcel($project_id, $fecha_inicial, $fecha_fin, $employee_id)
    {
        ...
        /** @var DataTrackingLaborRepository $dataTrackingLaborRepo */
        $dataTrackingLaborRepo = $this->getDoctrine()->getRepository(DataTrackingLabor::class);
        $lista = $dataTrackingLaborRepo->ListarEmployeesDeProject($project_id, $fecha_inicial, $fecha_fin, $employee_id);
        ...
        foreach ($grupos as $grupo) {
            ...
            foreach ($grupo as $entry) {
                if ('lead' === strtolower($entry['role']) && !empty($entry['color'])) {
    ...
    public function TotalReporteEmployees($sSearch, $employee_id, $project_id, $fecha_inicial, $fecha_fin)
    {
        /** @var DataTrackingLaborRepository $dataTrackingLaborRepo */
        $dataTrackingLaborRepo = $this->getDoctrine()->getRepository(DataTrackingLabor::class);
        $total = $dataTrackingLaborRepo->TotalReporteEmployees($sSearch, $employee_id, $project_id, $fecha_inicial, $fecha_fin);
