...
final class ItemActualizarRequest implements AdminHttpRequestDtoInterface
{
    #[Assert\NotBlank]
    ...
    {
        $d = new self();
        $iid = $request->get('item_id');
        $d->item_id = \is_string($iid) || is_numeric($iid) ? (string) $iid : null;
