# Cálculo de Bon y de X/Y (Boned) en Invoices

## 📋 Índice

1. [Cálculo de Bon en Invoices](#cálculo-de-bon-en-invoices) **(principal)**
2. [Conceptos Básicos (bone/boned)](#conceptos-básicos-boneboned)
3. [Cálculo de X (Proporción de BONED)](#cálculo-de-x-proporción-de-boned)
4. [Cálculo de Y (Monto Proporcional)](#cálculo-de-y-monto-proporcional)
...
## Cálculo de Bon en Invoices

### Objetivo

Definir la lógica para calcular **Bon**, **Bon Quantity** y **Bon Amount** en los invoices de un proyecto, asegurando que el Bon total del proyecto **nunca se exceda**.

### Definiciones

-  **Bon General del Proyecto** (`bon_general`): Valor total del Bon asignado al proyecto (ej: -1850). Se configura en el proyecto (pestaña General, campo "Bon General").
-  **Bon Quantity**: Porcentaje del Bon General que se desea aplicar a un invoice. **Rango válido: 0 a 1**.
-  **Bon Amount**: Monto monetario del Bon aplicado a un invoice.

### Regla principal

**La suma acumulada de todos los Bon Quantity aplicados no puede ser mayor que 1.**

-  Cada invoice consume parte del Bon disponible.
-  El último invoice solo puede usar el remanente, aunque solicite más.

### Variables de control

-  `bonGeneral`: Bon total del proyecto (tabla `project`, columna `bon_general`)
-  `bonQuantityUsed`: Bon Quantity ya utilizado (acumulado al recorrer invoices en orden)
-  `MAX_BON_QUANTITY = 1`

### Fórmulas

```
Bon Quantity Aplicado = min(Bon Quantity Solicitado, 1 - Bon Quantity Usado)
Bon Amount = Bon General * Bon Quantity Aplicado
```

### Flujo de cálculo por invoice

1. Ordenar todos los invoices del proyecto por `start_date` e `invoice_id`.
2. Para cada invoice:
   -  Si `bonQuantityUsed >= 1`: `bon_quantity = 0`, `bon_amount = 0`, continuar.
   -  `disponible = 1 - bonQuantityUsed`
   -  `aplicado = min(solicitado, disponible)`
   -  `invoice.bon_quantity = aplicado`
   -  `invoice.bon_amount = bon_general * aplicado`
   -  `bonQuantityUsed += aplicado`

### Validaciones obligatorias

-  Nunca permitir `bonQuantityUsed > 1`.
-  La suma de todos los Bon Amount debe ser igual al Bon General (cuando se usa todo el Bon).
-  Los invoices posteriores al agotamiento del Bon reciben Bon Quantity = 0 y Bon Amount = 0.

### Implementación

-  **Backend**: `InvoiceService::RecalcularBonProyecto($project_id)` recorre los invoices del proyecto en orden, aplica las fórmulas y persiste `bon_quantity` y `bon_amount` en cada invoice. Se llama al guardar un invoice y al cargar datos de un invoice.
-  **Base de datos**:
   -  `project.bon_general` (decimal 18,2, nullable)
   -  `invoice.bon_quantity_requested` (decimal 10,6, nullable) — lo que el usuario solicita (0–1)
   -  `invoice.bon_quantity` (decimal 10,6, nullable) — cantidad aplicada (calculada)
   -  `invoice.bon_amount` (decimal 18,2, nullable) — Bon General × bon_quantity (calculado)
-  **Frontend**: En la pantalla de invoice se muestran Bon General (solo lectura), Bon Qty Requested (input 0–1), Bon Qty Applied (solo lectura), Bon Amount (solo lectura). Al guardar se envía `bon_quantity_requested`.

### Ejemplo práctico

-  **Bon General** = -1850, **Bon Quantity Used** = 0.

| Invoice | Bon Qty Solicitado | Disponible | Bon Qty Aplicado | Bon Amount | Bon Qty Used |
| ------- | ------------------ | ---------- | ---------------- | ---------- | ------------ |
| 1       | 0.97               | 1          | 0.97             | -1794.50   | 0.97         |
| 2       | 0.20               | 0.03       | 0.03             | -55.50     | 1            |
| 3       | 0.50               | 0          | 0                | 0          | 1            |

---

## Conceptos Básicos (bone/boned)
...
### Cálculo de Bon (nuevo)

-  **database/cambios_constructora_bon_06_02.sql**: ALTER para `project.bon_general`, `invoice.bon_quantity_requested`, `invoice.bon_quantity`, `invoice.bon_amount`
-  **src/Entity/Project.php**: Propiedad `bonGeneral` y getters/setters
-  **src/Entity/Invoice.php**: Propiedades `bonQuantityRequested`, `bonQuantity`, `bonAmount` y getters/setters
-  **src/Utils/Admin/InvoiceService.php**: `RecalcularBonProyecto()`, uso en `CargarDatosInvoice`, `ActualizarInvoice`, `SalvarInvoice`
-  **src/Utils/Admin/ProjectService.php**: `bon_general` en `ListarItemsParaInvoice` y en `CargarDatos`; parámetro en `SalvarProject` y `ActualizarProject`
-  **src/Controller/Admin/InvoiceController.php**: Parámetro `bon_quantity_requested` al guardar
-  **src/Controller/Admin/ProjectController.php**: Parámetro `bon_general` al guardar proyecto; `bon_general` en respuesta de `listarItemsParaInvoice`
-  **templates/admin/invoice/index.html.twig**: Campos Bon General, Bon Qty Requested, Bon Qty Applied, Bon Amount
-  **templates/admin/block/modal-invoice.html.twig**: Mismos campos Bon en el modal
-  **templates/admin/project/index.html.twig**: Campo Bon General (pestaña Prevailing Wage)
-  **public/bundles/metronic8/js/pages/invoices.js**: Lectura/escritura de campos Bon y envío de `bon_quantity_requested`
-  **public/bundles/metronic8/js/components/modal-invoice.js**: Bon en modal y envío de `bon_quantity_requested`
-  **public/bundles/metronic8/js/pages/projects.js**: Campo `bon_general` en formulario de proyecto
