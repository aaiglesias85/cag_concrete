...
| 32 | `src/Controller/Admin/ProjectController.php` | `listar` → `ListarYTotalProjectsAdmin`; `listarDataTracking` → `ListarDataTrackingsParaProjectTab`; `eliminar` / `eliminarProjects` / `cargarDatos` pasan DTO al servicio; `salvar` → `SalvarProject`; `actualizar` → `ActualizarProject`; otras acciones grandes (sub-recursos, notas, etc.) pueden seguir parciales |
