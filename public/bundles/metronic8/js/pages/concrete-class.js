var ConcreteClass = (function () {
   ...
   var initTable = function () {
      const table = '#concrete-class-table-editable';
      // datasource
      const datasource = DatatableUtil.getDataTableDatasource(`concrete-class/listar`);
   ...
   var getColumnsTable = function () {
      ...
      columns.push({ data: 'name' }, { data: 'status' }, { data: null });
   ...
   var handleSearchDatatable = function () {
      const filterSearch = document.querySelector('#lista-concrete-class [data-table-filter="search"]');
      let debounceTimeout;
   ...
   var exportButtons = () => {
      const documentTitle = 'Concrete Classes';
      var table = document.querySelector('#concrete-class-table-editable');
      ...
      var buttons = new $.fn.dataTable.Buttons(table, {
         ...
         .container()
         .appendTo($('#concrete-class-table-editable-buttons'));

      // Hook dropdown menu click event to datatable export buttons
      const exportButtons = document.querySelectorAll('#concrete-class_export_menu [data-kt-export]');
   ...
   var actualizarRecordsSeleccionados = function () {
      ...
      if (selectedData.length > 0) {
         $('#btn-eliminar-concrete-class').removeClass('hide');
      } else {
         $('#btn-eliminar-concrete-class').addClass('hide');
      }
   };
   ...
   var initWizard = function () {
      $(document).off('click', '#form-concrete-class .wizard-tab');
      $(document).on('click', '#form-concrete-class .wizard-tab', function (e) {
         e.preventDefault();
   ...
   var initAccionNuevo = function () {
      $(document).off('click', '#btn-nuevo-concrete-class');
      $(document).on('click', '#btn-nuevo-concrete-class', function (e) {
         btnClickNuevo();
      });
      ...
      function btnClickNuevo() {
         ...
         KTUtil.find(KTUtil.get('form-concrete-class'), '.card-label').innerHTML = 'New Concrete Class:';
   ...
   var mostrarForm = function () {
      KTUtil.removeClass(KTUtil.get('form-concrete-class'), 'hide');
      KTUtil.addClass(KTUtil.get('lista-concrete-class'), 'hide');
   };
   ...
   var initAccionChange = function () {
      ...
      $(document).off('change', '#estadoactivo');
      $(document).on('change', '#estadoactivo', function (e) {
         event_change = true;
      });
   ...
   var cerrarFormsConfirmated = function () {
      resetForms();
      $('#form-concrete-class').addClass('hide');
      $('#lista-concrete-class').removeClass('hide');
   };
   //Editar
   var initAccionEditar = function () {
      $(document).off('click', '#concrete-class-table-editable a.edit');
      $(document).on('click', '#concrete-class-table-editable a.edit', function (e) {
         e.preventDefault();
         resetForms();

         var concrete_class_id = $(this).data('id');
         $('#concrete_class_id').val(concrete_class_id);
   ...
   var initAccionEliminar = function () {
      $(document).off('click', '#concrete-class-table-editable a.delete');
      $(document).on('click', '#concrete-class-table-editable a.delete', function (e) {
         e.preventDefault();
      ...
      $(document).off('click', '#btn-eliminar-concrete-class');
      $(document).on('click', '#btn-eliminar-concrete-class', function (e) {
         btnClickEliminar();
      });
      ...
      function btnClickEliminar() {
         var ids = DatatableUtil.getTableSelectedRowKeys('#concrete-class-table-editable').join(',');
         if (ids != '') {
            // mostar modal
            ModalUtil.show('modal-eliminar-seleccion', { backdrop: 'static', keyboard: true });
         } else {
            toastr.error('Select concrete classes to delete', '');
         }
      }

      function btnClickModalEliminar() {
         var concrete_class_id = rowDelete;

         var formData = new URLSearchParams();
         formData.set('concrete_class_id', concrete_class_id);

         BlockUtil.block('#lista-concrete-class');

         axios
            .post('concrete-class/eliminar', formData, { responseType: 'json' })
            .then(function (res) {
               if (res.status === 200 || res.status === 201) {
                  var response = res.data;
                  if (response.success) {
            ...
            .catch(MyUtil.catchErrorAxios)
            .then(function () {
               BlockUtil.unblock('#lista-concrete-class');
            });
      }
      ...
      function btnClickModalEliminarSeleccion() {
         ...
         axios
            .post('concrete-class/eliminarVarios', formData, { responseType: 'json' })
            .then(function (res) {
               if (res.status === 200 || res.status === 201) {
                  var response = res.data;
                  if (response.success) {
