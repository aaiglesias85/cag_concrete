var Districts = function () {
    ...
    var initTable = function () {
        const table = "#district-table-editable";
        // datasource
        const datasource = DatatableUtil.getDataTableDatasource(`district/listar`);
    ...
    var handleSearchDatatable = function () {
        const filterSearch = document.querySelector('#lista-district [data-table-filter="search"]');
        let debounceTimeout;
    ...
    var exportButtons = () => {
        const documentTitle = 'District';
        var table = document.querySelector('#district-table-editable');
        ...
        }).container().appendTo($('#district-table-editable-buttons'));

        // Hook dropdown menu click event to datatable export buttons
        const exportButtons = document.querySelectorAll('#district_export_menu [data-kt-export]');
    ...
    var actualizarRecordsSeleccionados = function () {
        ...
        if (selectedData.length > 0) {
            $('#btn-eliminar-district').removeClass('hide');
        } else {
            $('#btn-eliminar-district').addClass('hide');
        }
    }
    ...
    var resetForms = function () {
        ...
        // reset form
        MyUtil.resetForm("district-form");
    ...
    var validateForm = function () {
        ...
        //Validacion
        var form = KTUtil.get('district-form');
    ...
    var initAccionNuevo = function () {
        ...
        function btnClickNuevo() {
            ...
            KTUtil.find(KTUtil.get('form-district'), '.card-label').innerHTML = "New District:";
    ...
    var mostrarForm = function () {
        KTUtil.removeClass(KTUtil.get('form-district'), 'hide');
        KTUtil.addClass(KTUtil.get('lista-district'), 'hide');
    }
    ...
    var initAccionChange = function () {
        ...
        $(document).on('click', "#btn-exit-save-and-close", function (e) {
            ...
            $('#btn-salvar-district').trigger('click');
        });
    ...
    var cerrarFormsConfirmated = function () {
        resetForms();
        $('#form-district').addClass('hide');
        $('#lista-district').removeClass('hide');
    };
    //Editar
    var initAccionEditar = function () {
        ...
        function editRow(district_id) {
            ...
            axios.post("district/cargarDatos", formData, {responseType: "json"})
                .then(function (res) {
                    if (res.status === 200 || res.status === 201) {
                        var response = res.data;
                        if (response.success) {
            ...
            function cargarDatos(district) {

                KTUtil.find(KTUtil.get("form-district"), ".card-label").innerHTML = "Update District: " + district.description;

                $('#description').val(district.description);

                $('#estadoactivo').prop('checked', district.status);
    ...
    var initAccionEliminar = function () {
        ...
        function btnClickEliminar() {
            var ids = DatatableUtil.getTableSelectedRowKeys('#district-table-editable').join(',');
            if (ids != '') {
                // mostar modal
        ...
        function btnClickModalEliminar() {
            ...
            BlockUtil.block('#lista-district');

            axios.post("district/eliminar", formData, { responseType: "json" })
                .then(function (res) {
                    if (res.status === 200 || res.status === 201) {
                        var response = res.data;
                        if (response.success) {
                ...
                .catch(MyUtil.catchErrorAxios)
                .then(function () {
                    BlockUtil.unblock("#lista-district");
                });
        };
        ...
        function btnClickModalEliminarSeleccion() {
            ...
            axios.post("district/eliminarDistricts", formData, { responseType: "json" })
                .then(function (res) {
                    if (res.status === 200 || res.status === 201) {
                        var response = res.data;
                        if (response.success) {
