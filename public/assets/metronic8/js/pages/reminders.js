var Reminders = (function () {
   ...
   var handleSearchDatatable = function () {
      ...
      $(document).on(
         "keyup",
         '#lista-reminder [data-table-filter="search"]',
         function (e) {
   ...
   var exportButtons = () => {
      ...
      var buttons = new $.fn.dataTable.Buttons(table, {
         ...
         .container()
         .appendTo($("#reminder-table-editable-buttons"));

      // Hook dropdown menu click event to datatable export buttons
      const exportButtons = document.querySelectorAll(
         "#reminder_export_menu [data-kt-export]",
      );
   ...
   var initAccionNuevo = function () {
      ...
      function btnClickNuevo() {
         ...
         KTUtil.find(KTUtil.get("form-reminder"), ".card-label").innerHTML =
            "New Reminder:";
   ...
   var initAccionSalvar = function () {
      ...
      function btnClickSalvarForm() {
         ...
         if (validateForm() && usuarios_id.length > 0 && !bodyIsEmpty) {
            ...
            var salvarUrl =
               reminder_id && String(reminder_id).trim() !== ""
                  ? "reminder/actualizar"
                  : "reminder/salvar";
   ...
   var initAccionChange = function () {
      ...
      $(document).on("click", "#btn-exit-save-and-close", function (e) {
         ...
         $("#btn-salvar-reminder").trigger("click");
      });
   ...
   var initAccionEditar = function () {
      ...
      function editRow(reminder_id) {
         ...
         axios
            .post("reminder/cargarDatos", formData, { responseType: "json" })
            .then(function (res) {
               if (res.status === 200 || res.status === 201) {
                  var response = res.data;
                  if (response.success) {
         ...
         function cargarDatos(reminder) {
            KTUtil.find(KTUtil.get("form-reminder"), ".card-label").innerHTML =
               "Update Reminder: " + reminder.subject;
            ...
            for (var i = 0; i < destinatarios.length; i++) {
               $(select).append(
                  new Option(
                     `${destinatarios[i].nombre}<${destinatarios[i].email}>`,
                     destinatarios[i].usuario_id,
                     false,
                     false,
                  ),
               );
   ...
   var initAccionEliminar = function () {
      ...
      $(document).on(
         "click",
         "#reminder-table-editable a.delete",
         function (e) {
            e.preventDefault();
      ...
      function btnClickModalEliminar() {
         ...
         axios
            .post("reminder/eliminar", formData, { responseType: "json" })
            .then(function (res) {
               if (res.status === 200 || res.status === 201) {
                  var response = res.data;
                  if (response.success) {
      ...
      function btnClickModalEliminarSeleccion() {
         ...
         axios
            .post("reminder/eliminarReminders", formData, {
               responseType: "json",
            })
            .then(function (res) {
               if (res.status === 200 || res.status === 201) {
