var OverheadPrice = function () {
    ...
    var initTable = function () {
        const table = "#overhead-table-editable";
        // datasource
        const datasource = DatatableUtil.getDataTableDatasource(`overhead-price/listar`);
    ...
    var handleSearchDatatable = function () {
        const filterSearch = document.querySelector('#lista-overhead [data-table-filter="search"]');
        let debounceTimeout;
    ...
    var exportButtons = () => {
        const documentTitle = 'Overhead Price';
        var table = document.querySelector('#overhead-table-editable');
        ...
        }).container().appendTo($('#overhead-table-editable-buttons'));

        // Hook dropdown menu click event to datatable export buttons
        const exportButtons = document.querySelectorAll('#overhead_export_menu [data-kt-export]');
    ...
    var actualizarRecordsSeleccionados = function () {
        ...
        if (selectedData.length > 0) {
            $('#btn-eliminar-overhead').removeClass('hide');
        } else {
            $('#btn-eliminar-overhead').addClass('hide');
        }
    }
    ...
    var resetForms = function () {
        // reset form
        MyUtil.resetForm("overhead-form");
    ...
    var validateForm = function () {
        ...
        //Validacion
        var form = KTUtil.get('overhead-form');
    ...
    var initAccionNuevo = function () {
        ...
        function btnClickNuevo() {
            ...
            KTUtil.find(KTUtil.get('form-overhead'), '.card-label').innerHTML = "New Overhead Price:";
    ...
    var mostrarForm = function () {
        KTUtil.removeClass(KTUtil.get('form-overhead'), 'hide');
        KTUtil.addClass(KTUtil.get('lista-overhead'), 'hide');
    }
    ...
    var initAccionSalvar = function () {
        ...
        function btnClickSalvarForm() {
            ...
            if (validateForm() ) {
                ...
                formData.set("overhead_id", overhead_id);
                ...
                axios.post(salvarUrl, formData, {responseType: "json"})
                    ...
                    .then(function () {
                        BlockUtil.unblock("#form-overhead");
                    });
            }
        };
    }
    ...
    var initAccionChange = function () {
        ...
        $(document).on('click', "#btn-exit-save-and-close", function (e) {
            ...
            $('#btn-salvar-overhead').trigger('click');
        });
    ...
    var cerrarFormsConfirmated = function () {
        resetForms();
        $('#form-overhead').addClass('hide');
        $('#lista-overhead').removeClass('hide');
    };
    //Editar
    var initAccionEditar = function () {
        ...
        function editRow(overhead_id) {
            ...
            axios.post("overhead-price/cargarDatos", formData, {responseType: "json"})
                .then(function (res) {
                    if (res.status === 200 || res.status === 201) {
                        var response = res.data;
                        if (response.success) {
            ...
            function cargarDatos(overhead) {

                KTUtil.find(KTUtil.get("form-overhead"), ".card-label").innerHTML = "Update Overhead Price: " + overhead.name;

                $('#name').val(overhead.name);
                NumberUtil.setFormattedValue('#price', overhead.price, { decimals: 2 });
    ...
    var initAccionEliminar = function () {
        ...
        function btnClickEliminar() {
            var ids = DatatableUtil.getTableSelectedRowKeys('#overhead-table-editable').join(',');
            if (ids != '') {
                // mostar modal
        ...
        function btnClickModalEliminar() {
            ...
            BlockUtil.block('#lista-overhead');

            axios.post("overhead-price/eliminarOverhead", formData, { responseType: "json" })
                .then(function (res) {
                    if (res.status === 200 || res.status === 201) {
                        var response = res.data;
                        if (response.success) {
                ...
                .catch(MyUtil.catchErrorAxios)
                .then(function () {
                    BlockUtil.unblock("#lista-overhead");
                });
        };
        ...
        function btnClickModalEliminarSeleccion() {
            ...
            axios.post("overhead-price/eliminarOverheads", formData, { responseType: "json" })
                .then(function (res) {
                    if (res.status === 200 || res.status === 201) {
                        var response = res.data;
                        if (response.success) {
