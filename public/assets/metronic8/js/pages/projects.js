var Projects = (function () {
   ...
   var resetForms = function (reset_wizard = true) {
      ...
      actualizarTableListaInspectorsDataTracking();
   ...
   var cargarHistorialUnpaidQtyCompletion = function (project_item_id) {
      ...
      $('#modal-change-order-history .modal-title').text('Unpaid qty history');
      axios
   ...
   var handlePaidQtyOverrideHistoryCompletion = function () {
      $(document).off(
         'click',
         '#lista-items-completion .paid-qty-override-history-icon-completion, #lista-items-completion-detalle .paid-qty-override-history-icon-completion'
      );
      $(document).on(
         'click',
         '#lista-items-completion .paid-qty-override-history-icon-completion, #lista-items-completion-detalle .paid-qty-override-history-icon-completion',
         function (e) {
            ...
            var project_item_id = $(this).data('project-item-id');
            if (project_item_id) {
               cargarHistorialPaidQtyOverrideCompletion(project_item_id);
            }
         }
      );
   };

   var cargarHistorialPaidQtyOverrideCompletion = function (project_item_id) {
      BlockUtil.block('#modal-change-order-history .modal-content');
      $('#modal-change-order-history .modal-title').text('Paid qty override history');
      axios
         .get('project/listarHistorialPaidQtyOverridePorProjectItem', { params: { project_item_id: project_item_id }, responseType: 'json' })
         .then(function (res) {
            if (res.status === 200 || res.status === 201) {
               var response = res.data;
               if (response.success) {
                  ...
                  var html = '';
                  if (historial.length === 0) {
                     html = '<div class="alert alert-info">No paid qty override history for this item.</div>';
                  } else {
                     html = '<ul class="list-unstyled">';
                     historial.forEach(function (item) {
                        html += '<li class="mb-2"><i class="fas fa-circle text-primary me-2" style="font-size: 8px;"></i>' + item.mensaje + '</li>';
   ...
   var cargarHistorialChangeOrder = function (project_item_id, filterType) {
      BlockUtil.block('#modal-change-order-history .modal-content');
      $('#modal-change-order-history .modal-title').text('Change Order History');
      axios
   ...
   // Inspectors linked via data tracking (same rule as Projects tab on inspector)
   var inspectors_datatracking = [];
   var oTableInspectorsDataTracking;
   var initTableInspectorsDataTracking = function () {
      const table = '#project-inspectors-datatracking-table';
      ...
      const language = DatatableUtil.getDataTableLenguaje();
      oTableInspectorsDataTracking = DatatableUtil.initSafeDataTable(table, {
      ...
      $(document).off('keyup', '#lista-project-inspectors-datatracking [data-table-filter="search"]');
      $(document).on('keyup', '#lista-project-inspectors-datatracking [data-table-filter="search"]', function (e) {
         if (oTableInspectorsDataTracking) {
            oTableInspectorsDataTracking.search(e.target.value).draw();
         }
      });
   };
   var actualizarTableListaInspectorsDataTracking = function () {
      if (oTableInspectorsDataTracking) {
         oTableInspectorsDataTracking.destroy();
      }
      initTableInspectorsDataTracking();
   };
   ...
   var agruparItemsPorChangeOrderCompletion = function (items) {
      ...
      if (items_change_order.length > 0 && items_regulares.length > 0) {
         resultado.push({
            ...
            diff_qty: null,
            diff_amt: null,
            has_paid_qty_override_history: false,
         });
      }
   ...
   var initTableItemsCompletion = function () {
      ...
      let columnDefs = [
         ...
         {
            // Columnas de Cantidad (Qty, Qty Completed, Invoiced Qty) — Paid Qty (10) tiene render propio con historial override
            targets: [2, 5, 8],
            className: 'text-end',
            render: function (data, type, row) {
               return data ? $.fn.dataTable.render.number(',', '.', 2, '').display(data) : '0.00';
         ...
         {
            targets: 10, // Paid Qty (agregado con override) + historial invoice_item_override_payment_paid_qty_history
            className: 'text-end',
            render: function (data, type, row) {
               if (row.isGroupHeader) return '';
               var numHtml = data !== null && data !== undefined && data !== ''
                  ? $.fn.dataTable.render.number(',', '.', 2, '').display(data)
                  : '0.00';
               if (type === 'display' && row.has_paid_qty_override_history && row.project_item_id) {
                  var icon =
                     '<i class="fas fa-plus-circle text-primary ms-1 cursor-pointer paid-qty-override-history-icon-completion" style="cursor: pointer; display: inline-block;" data-project-item-id="' +
                     row.project_item_id +
                     '" title="Paid qty override history"></i>';
                  return '<div class="d-flex align-items-center justify-content-end gap-1">' + numHtml + icon + '</div>';
               }
               return numHtml;
            },
         },
      ...
      handlePaidQtyOverrideHistoryCompletion();
